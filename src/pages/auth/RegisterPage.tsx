...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err.code === 'auth/email-already-in-use'
        ? t('firebaseErrors.emailAlreadyInUse')
        : err.code === 'auth/weak-password'
        ? t('firebaseErrors.weakPassword')
        : t('firebaseErrors.unexpected');
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      toast.success(t('auth.accountCreatedShort'));
      navigate('/');
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-dark-950 flex items-center justify-center px-4 py-12 relative">
      ...
      <div className="w-full max-w-md animate-fade-in">
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-light text-white tracking-widest uppercase">{t('common.register')}</h1>
        </div>
        ...
        <div className="bg-dark-900 border border-dark-800 rounded-none p-8 space-y-6">
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-3 bg-dark-900 text-dark-500">{t('auth.orRegisterWithEmail')}</span>
            </div>
          </div>
        ...
        <p className="text-center text-dark-400 text-sm mt-6">
          <Link to="/login" className="text-astro-400 hover:text-astro-300 font-medium">
            {t('common.login')}
          </Link>
        </p>
      </div>
